...
VALUES
  ...
  ('HAIR_BACK', 'Cheveux en arrière', FALSE, 7, TRUE),
  ('EYE_CLOSEUP', 'Gros plan œil', TRUE, 7, TRUE),
...
CREATE TABLE IF NOT EXISTS public.scan_sessions (
  ...
  required_asset_count INTEGER DEFAULT 7 NOT NULL,
...
-- Client-side error telemetry (inserts via API service role only; RLS on, no end-user policies)
...
-- Purge client_error_reports: p_delete_all = true deletes all rows; false deletes rows older than 90 days.
...
-- Weekly stale purge (Sunday 04:00 UTC). Requires pg_cron enabled on the project.
...
-- get_recent_scan_status: returns the user's latest manual-rescan session
-- with recent app activity. Readiness is calculated within that one session
-- so the web app never tries to launch an incomplete `manual_rescan`.
